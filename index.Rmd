...
```{r maps, echo=FALSE, out.width="100%"}
 ...
 map <- mapview(filter(sonde_locations, network == "Upper" & site_abbreviation %nin% c("PENN", "LBEA")), layer.name = "CSU X CU Upper",col.regions = "purple", cex = 8)+
  mapview(filter(sonde_locations, site_abbreviation %in% c("PENN", "LBEA")), layer.name = "Burned Study Tributaries",col.regions = "red", cex = 8)+
        mapview(filter(sonde_locations, network == "Lower" & site_abbreviation %nin% c("Lincoln","Timberline","Prospect", "Archery")), layer.name = "CSU Lower",col.regions = "cyan", cex = 6)+
  mapview(filter(sonde_locations, network == "Lower" & site_abbreviation %in% c("Lincoln","Timberline","Prospect", "Archery")), layer.name = "CSU X CU Lower",col.regions = "blue", cex = 8)+
  mapview(filter(sonde_locations, site_abbreviation %in% c("SpringCreek", "BoxCreek")), layer.name = "Lower Network Tributaries",col.regions = "green", cex = 8)
