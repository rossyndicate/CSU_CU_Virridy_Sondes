...
lab_data_at_sensors <- most_recent_chem%>%
  ...
  rm(sites, correlated_df, merge_grab_sensor)
```
...
subset <- filter(tidy_correlated_df, site %in% c("joei", "pfal", "sfm"))

ggplot(tidy_correlated_df, aes(x = SC, y = `Specific Conductivity`, color = site))+
   geom_point()+
    #geom_smooth(method = "lm")+
  #add a one to one line
  geom_abline(intercept = 0, slope = 1, color = "red")+
  labs( x= "Lab SC(µS/cm)", y = " Sensor SC (µS/cm)")
